...
function submitDrawing() {
    ...
    validateSvg(drawingCanvas.level.goalImg, function (errors) {
        if (errors.length == 0){
            ...
            allDone = checkCompletion(); //TODO: figure out if we should use this
            ...
            if (thereAreMoreLevels) {
                ...
                levelPopup.innerHTML = '<div class="popup-button" id="popup-close-button" onclick="closeCompletionPopup()">X</div><div id="well-done">Well done!</div><div class="popup-button" id="popup-next-button" onclick="nextLevel()">Next Level</div>';
                ...
                levelPopup.classList.remove('popup-hidden');
                levelPopup.innerHTML = '<div class="popup-button" id="popup-close-button" onclick="closeCompletionPopup()">X</div><div id="well-done-final">Well done!</div><div id="final-message">That\'s the last level for now, but you can keep doodling in our free-draw sandbox.</div><div class="popup-button" id="popup-sandbox-button" onclick="selectSandbox()">Go to Sandbox</div>';
                mainCanvas.classList.remove('canvas-active');
                drawingCanvas.canvasActive = false;
            }
        } 
